...
(function () {
  ...
  function preloadMatchImages(matches) {
    ...
    if (matches.length > 0) {
      ...
      let rawImg1 = top1.best_matching_image_url || top1.image_path || (top1.all_angles && top1.all_angles[0] ? top1.all_angles[0].image_path : '');
  ...
  function renderMatchResults(data) {
    ...
    matches.forEach((m, idx) => {
      ...
      const designId = m.design_id || m.id || "N/A";
      
      const catalogRef = state.catalog.find(c => (c.design_id || c.id) === designId) || {};
      const designName = m.design_name || m.name || catalogRef.name || "Footwear Design";
      const category = m.category || catalogRef.category || "Footwear";
      const materialsText = (m.materials || catalogRef.materials || "Standard Synthetic Leather / Rubber").trim();
      const locationText = (m.shelf_location || catalogRef.shelf_location || "Warehouse A - Rack 03 - Shelf B-02").trim();
      ...
      card.innerHTML = `
        ...
        <div class="farma-shelf-chip">
          <span>Farma Shelf: <strong>${escapeHtml(farmaShelfText)}${(m.drawer || catalogRef.drawer) ? ` • Drawer: ${escapeHtml((m.drawer || catalogRef.drawer).trim())}` : ''}${(m.slot || catalogRef.slot) ? ` • Slot: ${escapeHtml((m.slot || catalogRef.slot).trim())}` : ''}</strong></span>
        </div>` : ''}
