"""
Rebuild FAISS index by re-embedding all catalog reference photos through the color-aware EmbeddingEngine.
...
def rebuild_color_index():
    logger.info("=== Rebuilding FAISS Index — Full Color RGB Embeddings ===")
    db.init_db()
    ...
    for idx, ref in enumerate(all_refs):
        ...
        if not img_path.exists():
            logger.warning(f"  [{idx+1}/{len(all_refs)}] SKIP — image not found: {img_path}")
            continue

        try:
            assert_catalog_image_path(img_path)
        except Exception as e:
            logger.warning(f"  [{idx+1}/{len(all_refs)}] Guardrail warning for {img_path}: {e}")
        ...
        try:
            ...
            if (idx + 1) % 25 == 0 or (idx + 1) == len(all_refs):
                logger.info(f"  [{idx+1}/{len(all_refs)}] Processed {design_id} ({ref.get('angle', '')})")
        except Exception as e:
            logger.error(f"  [{idx+1}/{len(all_refs)}] [FAIL] Failed to embed {img_path}: {e}")
